...
def main():
    ...
    # allroles = pd.read_csv(Path(r'Z:\Chad Projects\Alerts\Production\assets\caching\allroles.csv'))
    # pers = pd.read_csv(Path(r'Z:\Chad Projects\Alerts\Production\assets\caching\pers.csv'))
    # viewperstaxid = pd.read_csv(Path(r'Z:\Chad Projects\Alerts\Production\assets\caching\viewperstaxid.csv'))
    ...
    src.cdutils.input_cleansing.enforce_schema(viewperstaxid, viewperstaxid_schema)
    ...
    loan_data = src.control_panel.criteria_flags(
        ...
        min_deposits = 250000, # Minimum deposits to be eligible to flag deposit decrease
    ...
    final_df.to_excel(file_path, index=False, engine='openpyxl', sheet_name='Sheet1')
    credit_score_data.to_excel(Path(r'\\10.161.85.66\Home\Share\Data & Analytics Initiatives\Project Management\Chad Projects\Alerts\Production\Output\credit_score_detail.xlsx'), index=False, engine='openpyxl', sheet_name='Sheet1')
