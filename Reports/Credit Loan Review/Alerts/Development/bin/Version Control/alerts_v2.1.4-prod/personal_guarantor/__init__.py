...
def append_credit_score(pers_data: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        df (pd.DataFrame)
    """
    xactus_file = pd.read_csv(Path(r'Z:\Chad Projects\Alerts\Production\assets\xactus\xactus_output.csv'))
    ...
    xactus_file = src.cdutils.input_cleansing.enforce_schema(xactus_file, xactus_file_schema)
